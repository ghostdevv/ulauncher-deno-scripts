# Based on BSD3-Clause Licensed code from Amjith Ramanujam
# https://github.com/amjith/fuzzyfinder/blob/0141f71fa371c9496ecfda1373820e7b202f975f/fuzzyfinder/main.py
# https://github.com/amjith/fuzzyfinder/blob/0141f71fa371c9496ecfda1373820e7b202f975f/LICENSE
...
def highlight_substring(
    ...
    if isinstance(highlight, str):
        if not (ansi_code := COLORS_TO_ANSI.get(highlight.lower())):
            raise ValueError(
                f"highlight, if a string, must be one of: {str(list(COLORS_TO_ANSI)).strip('[]')}"
            )
...
def fuzzyfinder(
    ...
    if highlight:
        results = (
            highlight_substring(input, result, highlight, ignore_case)
